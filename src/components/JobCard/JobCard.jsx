...
function JobCard(props) {
  ...
  return (
    <div className='mx-40 mb-4'>
      <div className='flex justify-between item-center px-6 py-4 bg-zinc-200 rounded-md border border-black shadow-lg hover:border-blue-500 hover:translate-y-1 hover:translate-scale-103'>
        <div className='flex flex-col items-start gap-3'>
            <h1 className='text-lg font-semibold'>{props.title} - {props.company}</h1>
            <p>{props.type} | {props.experience} | {props.location}</p>
            <div className='flex items-center gap-2'>
              {props.skills.map((skill)=>(
                <p key={skill} className='text-grey-500 py-1 px-2 rounded-md border border-black'>{skill}</p>
              ))}
            </div>
        </div>
